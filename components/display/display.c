...
// now the 4 edge lines.
  ...
  OldLinestoRender=LinestoRender;
    ...
    for (i=0; i<LinestoRender; i++ )
    {
      graphic_line(ORender[i].p0.x, ORender[i].p0.y - 30, ORender[i].p1.x, ORender[i].p1.y - 30, 0xFFFF);
      graphic_line(Render[i].p0.x, Render[i].p0.y - 30, Render[i].p1.x, Render[i].p1.y - 30, 0xAAAA);
    }
